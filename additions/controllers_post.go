...
func MakePostProcess(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	session := sessionManager.Load(r)
	var cUser User
	if UserId, err := session.GetInt("UserId"); err != nil {
		fmt.Println(err)
	...
	if r.ContentLength > maxFileSize {
		...
		http.Redirect(w, r, "/makePost", 303)
		return
	}
	...
	newPost := Post{Title: title, Text: text, By: cUser.Name}
	sentFlash["TitlePost"] = title
	sentFlash["Text"] = text
	sentFlash["SelectedCat"] = selectedCat
	sentFlash["Cat"] = madeCat
	sentFlash["Cats"] = getAllCats("", "")
	if warnings, cat = newPost.Validate(selectedCat, madeCat, "", tags); len(warnings) > 0 {
		sentFlash["Errs"] = warnings
		if err := session.PutObject(w, "sentFlash", sentFlash); err != nil {
			fmt.Println(err)
		}
		http.Redirect(w, r, "/makePost", 302)
		return
	}
	...
	if !missFile && len(FileHeader.Filename) != 0 {
		post, _, _, _ := getPostById(postId)
		picName := processPostPic(file)
		if affect := addPostPic(post, picName); affect < 1 {
			fmt.Println("post didn't get updated to get its pic")
		}
	}
	processTag(postId, tags)
